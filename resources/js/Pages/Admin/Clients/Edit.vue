<script setup>
import { ref } from 'vue'
import { Link as TheLink, usePage, useForm } from '@inertiajs/inertia-vue3'
      ...
      <div class="w-full max-w-7xl rounded-lg bg-white m-6 shadow-lg overflow-hidden">
        <nav class="flex py-4 px-6 justify-end border-b border-gray-100">
          ...
          >
            <span class="relative">
              <span
                v-if="form.errors[availableLocale.value]" 
                class="absolute -top-1 -right-5 flex flex-no-shrink items-center justify-center bg-red-500 text-white rounded-full text-xs h-4 w-4"
              >
                E
              </span>
          ...
          <form method="POST" @submit.prevent="submit">
            <div class="grid grid-cols-1 gap-6 p-6">
              ...
              <div
                ...
                :key="item.id" 
                class="grid grid-cols-3 gap-6 p-3 bg-gray-100/25 rounded"
              >
                <div>
                  ...
                  <TheCombo 
                    id="type" 
                    v-model="item.type" 
                    :options="itemTypes.map(itemType => ({ value: itemType.value, text: itemType.translations[locale] }))" 
                    class="block w-full" 
                  />
              ...
              <div class="flex w-full justify-end items-baseline space-x-8">
                <div 
                  v-if="form.isDirty" 
                  class="grow justify-self-start font-semibold text-red-600"
                >
                ...
                <button
                  ...
                  class="inline-block rounded border-2 border-blue-600 bg-blue-600 px-6 py-2 font-bold leading-tight text-white transition duration-150 ease-in-out hover:bg-blue-500 focus:outline-none focus:ring-0"
                >
                  Update Client
                </button>
              </div>
